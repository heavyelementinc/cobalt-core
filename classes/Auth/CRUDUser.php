...
class CRUDUser{
    ...
    function validate_all(){
        ...
        if(count($mutant) === 0) throw new \Exceptions\HTTP\BadRequest("No valid data submitted.");
        return $mutant;
    }
    ...
    function add_user($data){
        ...
        if($this->getInsertedCount() !== 1) throw new \Exceptions\HTTP\BadRequest("Failed to add user to database.");
        $validated_user['_id'] = $this->getInsertedId();
        return $validated_user;
    }

    function update_user($user_id,$data){
        if(empty($user_id)) throw new \Exceptions\HTTP\BadRequest("Missing _id from request.");
        try{
            $uid = new MongoDB\BSON\ObjectId($user_id);
        } catch(Exception $e){
            throw new \Exceptions\HTTP\BadRequest("The _id is malformed.");
        }
        $this->user_info = $data;
        ...
        if($result->getModifiedCount() !== 1) throw new \Exceptions\HTTP\BadRequest("Failed to update the user entry.");
        return $result;
    }
}