...
abstract class PostController extends Controller {
    ...
    public function admin_index() {
        ...
        $result = $this->postMan->find(...$this->params($this->postMan, [], [
    ...
    public function edit($id = null) {
        if(!$this->postMan) throw new Exception("You must manually initialize the PostController");
        $post = $this->postMan->findOne(['_id' => $this->postMan->__id($id)]);
        if(!$post) {
            $schema = $this->postMan->get_schema_name();
        ...
        add_vars([
            ...
            'pretty' => "<fold-out title=\"Raw Database Entry\"><pre>".htmlspecialchars(json_encode($post, JSON_PRETTY_PRINT))."</pre></fold-out>",
        ]);
