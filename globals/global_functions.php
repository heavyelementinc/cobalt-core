...
function youtube_embedder(string $html, DOMElement $img){
    $regExp = "/(<img src=\"".$img->getAttribute('src')."\")[^/>]*(/>|>)/";
...
function instagram_embedder(string $html, DOMElement $img) {
    $regExp = "/(<img src=\"".$img->getAttribute('src')."\")[^/>]*(/>|>)/";
...
function embed_from_img_tags($html) {
    ...
    foreach($imgTags as $img) {
        $src = parse_url($img->getAttribute('src'), PHP_URL_HOST);
        switch($src) {
            case "www.youtube.com":
            case "youtu.be":
            case "www.youtu.be":
            case "youtube.com":
                $mutable = youtube_embedder($mutable, $img);
                break;
            case "instagram.com":
            case "www.instagram.com":
                $mutable = instagram_embedder($mutable, $img);
                break;
        }
    }
    return $mutable;
